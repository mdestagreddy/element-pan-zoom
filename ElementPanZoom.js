    ...
    animLoop = () => {
      ...
      if (gesture.d == true && anim.a.finish == true) {
        if (gesture.length <= 1) {
          ...
          hh = ((gesture.newDistance - gesture.distance) / b) * gesture.c.scale;     
          th1x = (gesture.move1.x - gesture.start1.x);
          th2x = (gesture.move2.x - gesture.start2.x);
          tr.x = gesture.c.x + ((th1x + th2x) / 2) + gesture.tar.x * hh * gesture.c.scale;
          th1y = (gesture.move1.y - gesture.start1.y);
          th2y = (gesture.move2.y - gesture.start2.y);
          tr.y = gesture.c.y + ((th1y + th2y) / 2) + gesture.tar.y *  hh * gesture.c.scale;
